<div align="center">

# 🐼 DevPanda

### Browser-based utility toolkit for AI/ML and general developers

[![Live Site](https://img.shields.io/badge/🌐_Live_Site-devpanda.nullpoint.cloud-6366f1?style=for-the-badge)](https://devpanda.nullpoint.cloud)
[![Firebase](https://img.shields.io/badge/Hosted_on-Firebase-FFCA28?style=for-the-badge&logo=firebase&logoColor=black)](https://firebase.google.com)<br>
[![Vite](https://img.shields.io/badge/Built_with-Vite-646CFF?style=for-the-badge&logo=vite&logoColor=white)](https://vitejs.dev)
[![License: MIT](https://img.shields.io/badge/License-MIT-22c55e?style=for-the-badge)](LICENSE)

> No install. No signup. No backend. Just open and use.

</div>

---

## ✨ Features

- ⚡ **100% in-browser** — all processing happens client-side
- 🔒 **Zero data storage** — nothing leaves your machine
- 🆓 **Free forever** — no paywalls, no accounts
- 📱 **Responsive** — works on desktop and mobile

---

## 🛠️ General Dev Tools

| Tool | Description |
|------|-------------|
| 📋 JSON Formatter | Format, validate & beautify JSON |
| 🔑 Base64 Converter | Encode/decode Base64 strings |
| 🔍 Regex Tester | Test and debug regex patterns |
| 🔐 Hash Generator | MD5, SHA-1, SHA-256 hash generation |
| 🌐 URL Encoder | Encode/decode URLs |
| 📊 CSV ↔ JSON | Convert between CSV and JSON formats |
| 🎨 Color Picker | HEX/RGB/HSL color converter |
| 🖼️ Image Resizer | Resize images in-browser |
| 📦 Image Compressor | Reduce image size without changing resolution |
| 🔷 Image ↔ HEX | Convert images to HEX values |
| 🖊️ HTML Preview | Live HTML/CSS/JS editor with preview |
| 📐 Unit Converter | Length, weight, temperature, and more |
| ⏱️ Cron Parser | Parse and describe cron expressions |
| 📄 PDF / OCR Extractor | Extract text from PDFs and images |

## 🤖 AI / ML Tools

| Tool | Description |
|------|-------------|
| 🧮 Tensor Shape Calculator | Compute tensor shapes through NN layers |
| 🎮 GPU Memory Calculator | Estimate VRAM usage for model training |
| 🏷️ Label Converter | Convert YOLO ↔ COCO ↔ Pascal VOC |
| 📈 Training Analyzer | Analyze training logs and detect issues |
| 📉 LR Scheduler Visualizer | Visualize learning rate schedules |
| 🔢 Model Param Counter | Count model parameters by layer |
| 🖼️ Augmentation Previewer | Preview image augmentation effects |
| 📐 Detection Metrics | Calculate mAP, IoU, precision, recall |

---

## 🚀 Getting Started

```bash
# Clone the repo
git clone https://github.com/ChoiJaeWoon/devpanda.git
cd devpanda

# Install dependencies
npm install

# Start dev server
npm run dev

# Build for production
npm run build
```

---

## 🏗️ Tech Stack

| Layer | Technology |
|-------|-----------|
| Build Tool | [Vite](https://vitejs.dev) |
| Hosting | [Firebase Hosting](https://firebase.google.com) |
| Styling | Vanilla CSS |
| Fonts | [Google Fonts](https://fonts.google.com) — Inter |
| News Feed | [Hacker News API](https://github.com/HackerNews/API), [Dev.to API](https://developers.forem.com/api) |
| Link Preview | [Microlink API](https://microlink.io) (free tier) |
| Image Processing | Browser Canvas API |

---

## 📁 Project Structure

```
devpanda/
├── public/
│   ├── favicon.svg
│   ├── og-image.png
│   ├── robots.txt
│   └── sitemap.xml
├── src/
│   ├── components/     # Shared UI components (navbar, sidebar, footer)
│   ├── pages/          # Individual tool pages
│   ├── styles/pages/   # Per-page CSS
│   ├── utils/          # Router, helpers
│   ├── main.js         # App entry & route registration
│   └── style.css       # Global styles
├── index.html
└── vite.config.js
```

---

## 📬 Contact

Bug reports & feedback: [devpanda@nullpoint.cloud](mailto:devpanda@nullpoint.cloud)

---

<div align="center">

Made with ❤️ for developers · [devpanda.nullpoint.cloud](https://devpanda.nullpoint.cloud)

</div>
